...
PARQUET = Path(os.environ.get("ALLOC_PARQUET", DB.parent / "parquet"))
...
from backtest.walkforward import walk_forward, START_VALUE
...
# the research lab (extra optimisers / overlays / relaxed caps) only runs when ALLOC_LAB=1.
LAB = os.environ.get("ALLOC_LAB") == "1"
...
def make_target(method, tier, config):
    ...
    return lambda r: allocate(r, tier, config, method)
...
def _run(ret, target_fn, min_hist, rf=None):
    ...
    m["end_value"] = eq.iloc[-1] * START_VALUE
    m["profit"] = m["end_value"] - START_VALUE
    m["profit_pct"] = m["profit"] / START_VALUE
    m["cost_amount"] = sum(row["cost_amount"] for row in rr)
...
def main() -> None:
    ...
    for variant in VARIANTS:
        ...
        for uname, (cfg, ret) in universes.items():
            ...
            # the money-market sleeve is the risk-free rate; without it, Sharpe rewards cash
            rf = ret[CASH_NAME]
            rfs[uname] = rf
...
def _scoreboard(runs, n_trials) -> None:
    ...
    for (u, v, t, meth), (_, m) in runs.items():
        rows.append({"variant": v, "tier": t, "method": meth, "universe": u,
                     "start": START_VALUE, "end": round(m["end_value"]),
