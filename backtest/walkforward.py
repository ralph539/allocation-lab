...
def walk_forward(returns: pd.DataFrame, target_fn, min_history: int,
    ...
    for t in idx:
        ...
        day_ret = 0.0
        if h is not None:
            pnl += eq * START_VALUE * h * r          # today's P&L on holdings set at last rebalance
            wsum += h
            ndays += 1
            day_ret = float(h @ r)
            eq *= 1 + day_ret
        ...
        if t in rebal:
            ...
            val_at = eq * START_VALUE                 # value at close of t, before cost
            ...
            for i, sl in enumerate(cols):
                rebal_rows.append({
                    ...
                    "trade_pct": float(trade[i]), "trade_amount": float(trade[i] * val_at),
                    "cost_amount": float(rate * abs(trade[i]) * val_at) if breached else 0.0,
                    "breached": bool(breached)})
        if h is not None:
            peak = max(peak, eq)
            value_rows.append({"date": t, "book_value": eq * START_VALUE,
                               "daily_ret": day_ret, "drawdown": eq / peak - 1})
            curve.append(eq)
            dates.append(t)
    equity = pd.Series(curve, index=pd.DatetimeIndex(dates))
    attribution = [{"sleeve": cols[i], "pnl": float(pnl[i]),
                    "avg_weight": float(wsum[i] / ndays) if ndays else 0.0,
                    "contrib_return": float(pnl[i] / START_VALUE)} for i in range(len(cols))]
