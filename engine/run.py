...
from engine.io import load_config, load_returns, TIERS, DB
...
def main() -> None:
    ...
    _save(records)
    print(f"Saved engine_weights ({len(records)} rows) to {DB} and {DB.parent / 'parquet'}")


def _save(records: list) -> None:
    df = pd.DataFrame(records, columns=["tier", "method", "sleeve", "weight"])
    out = DB.parent / "parquet"
    out.mkdir(parents=True, exist_ok=True)
    con = duckdb.connect(str(DB))
    try:
        con.register("tmp", df)
        con.execute("CREATE OR REPLACE TABLE engine_weights AS SELECT * FROM tmp")
        con.execute(f"COPY engine_weights TO '{out / 'engine_weights.parquet'}' (FORMAT PARQUET)")
    finally:
        con.close()
