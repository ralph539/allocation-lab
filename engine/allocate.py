...
from engine.views import build_views, bl_posterior, momentum_stance
...
def _cvar_family(R, tier, config, band, regime, method) -> np.ndarray:
...
def _base_weights(R, tier, config, b, regime, method) -> np.ndarray:
    ...
    if method in ("mean_cvar", "mean_cvar_anchored", "black_litterman_mom", "trend_tilt"):
        return _cvar_family(R, tier, config, b, regime, method)
    raise ValueError(f"unknown method {method}")


def allocate(returns_upto_T: pd.DataFrame, tier: str, config, method: str = "mean_cvar",
             regime: str = "full", band: float = None, overlay: str = None) -> pd.Series:
    ...
    w = _base_weights(R, tier, config, b, regime, base)
    if ov:
