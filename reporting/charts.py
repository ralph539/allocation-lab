...
SHORT = {"Equity - Europe": "Europe", "Equity - US": "US",
         ...
         "Real assets / REITs / infrastructure": "Real assets", "Cash / money market": "Cash"}
def _short(s: str) -> str:
    # chart label: the short name where one is defined, else the part after the bucket
    return SHORT.get(s, s.split(" - ")[-1])
...
def draw_equity_drawdown(ax_eq, ax_dd, value, bench=None) -> None:
    d = value["date"]
    ax_eq.plot(d, value["book_value"], color=EQUITY_LINE, lw=1.4, label="This run", zorder=3)
    if bench is not None:
        b = bench.set_index("date").reindex(d)
        ax_eq.plot(d, b["book_value"], color=BAR, lw=1.1, ls="--", label="60/40 benchmark")
...
def draw_pnl(ax, attrib, scale=1.0) -> None:
    a = attrib.sort_values("pnl")
    ax.barh([_short(s)[:22] for s in a["sleeve"]], a["pnl"],
            color=[GAIN if v >= 0 else LOSS for v in a["pnl"]])
...
def pnl_alt(attrib, height=None):
    ...
    a["lab"] = [f"{v / 1e3:+,.0f}k" for v in a["pnl"]]
    a = a.sort_values("pnl", ascending=False)
    order = list(a["sleeve_s"])
    h = height or max(300, 27 * len(a))
    tt = [alt.Tooltip("sleeve_s:N", title="Sleeve"),
          alt.Tooltip("pnl:Q", format=",.0f", title=f"P&L {CCY}")]
    ...
    bars = alt.Chart(a).mark_bar(size=13).encode(
        y=y, x=x,
        color=alt.condition("datum.pnl >= 0", alt.value(GAIN), alt.value(LOSS)),
        tooltip=tt)
    lab_p = (alt.Chart(a[a["pnl"] >= 0])
             .mark_text(align="left", dx=5, font=FONT, fontSize=11.5, color="#565b61")
             .encode(y=y, x=alt.X("pnl:Q", axis=None), text="lab:N"))
    lab_n = (alt.Chart(a[a["pnl"] < 0])
