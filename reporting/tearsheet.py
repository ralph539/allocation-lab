...
REPORTS = Path(os.environ.get(
    "ALLOC_REPORTS", Path(__file__).resolve().parent.parent / "reports"))
...
def generate(tier: str, method: str, universe: str, variant: str = "house") -> Path:
    ...
    start, end = value["book_value"].iloc[0], value["book_value"].iloc[-1]
    profit, cost = end - start, rebal["cost_amount"].sum()

    fig = plt.figure(figsize=(8.27, 11.69))
    gs = fig.add_gridspec(100, 12, left=0.09, right=0.95, top=0.96, bottom=0.04)

    axt = fig.add_subplot(gs[0:6, :]); axt.axis("off")
    axt.text(0, 0.7, "Allocation Lab backtest", fontsize=15, fontweight="bold")
    ...
    rows = [("Start value", data.fmt_money(start), "Volatility", data.fmt_pct(m["ann_vol"])),
            ("End value", data.fmt_money(end), "Sharpe", f"{m['sharpe']:.2f}"),
            ("Total profit", f"{data.fmt_money(profit)} ({profit/start*100:.1f}%)",
             "Deflated Sharpe", f"{m['dsr']:.3f}"),
            ("CAGR", data.fmt_pct(m["ann_return"]), "Max drawdown", data.fmt_pct(m["max_dd"])),
            ("95% CVaR (daily)", data.fmt_pct(m["cvar95"]), "Total cost", data.fmt_money(cost)),
    ...
    axs.text(0, 0.45, f"Rebalance checks: {reb_dates}.  Trades executed: {trade_dates}.  "
             f"Total traded cost: {data.fmt_money(cost)}.  "
