...
st.set_page_config(page_title="Allocation Lab", layout="wide")
...
.al-brand {font-size: 15px; font-weight: 700; letter-spacing: .14em; color: #14161a;}
.al-brandsub {font-size: 12.5px; color: #6b7280; margin: 2px 0 16px;}
...
.al-kicker {font-size: 12px; font-weight: 600; letter-spacing: .08em;
  text-transform: uppercase; color: #6b7280; margin-bottom: 2px;}
.al-title {font-family: Georgia, "Times New Roman", serif; font-size: 34px;
  font-weight: 700; line-height: 1.12; color: #14161a; letter-spacing: -.005em;}
.al-desc {font-size: 14.5px; line-height: 1.45; color: #4b5157; max-width: 72ch;
  margin: 4px 0 0;}
.al-hero {display: flex; flex-wrap: wrap; gap: 12px 64px; margin-top: 18px;
  padding: 16px 0 14px; border-top: 3px solid #14161a; animation: al-in .18s ease-out;}
.al-hero .lbl, .al-strip .lbl {font-size: 11px; font-weight: 600;
  letter-spacing: .07em; text-transform: uppercase; color: #6b7280;
  margin-bottom: 4px; white-space: nowrap; cursor: default;}
.al-hero .big {font-size: 40px; font-weight: 700; line-height: 1; color: #14161a;
  letter-spacing: -.01em;}
.al-hero .big.dim {color: #6b7280; font-weight: 600;}
.al-hero .sub {font-size: 13.5px; color: #4b5157; margin-top: 6px;}
.pos {color: #2f855a; font-weight: 600;} .neg {color: #c53030; font-weight: 600;}
.al-strip {display: flex; flex-wrap: wrap; gap: 10px 40px; padding: 12px 0;
  border-top: 1px solid #e2e2db; border-bottom: 1px solid #e2e2db;
  animation: al-in .18s ease-out;}
.al-strip .val {font-size: 21px; font-weight: 600; color: #14161a; margin-top: 1px;}
.al-note {font-size: 12.5px; color: #6b7280; margin-top: 6px;}
.al-sec {display: flex; justify-content: space-between; align-items: baseline;
  font-size: 13px; font-weight: 700; letter-spacing: .05em; text-transform: uppercase;
  color: #14161a; margin: 30px 0 4px;}
.al-sec .hint {font-weight: 400; letter-spacing: 0; text-transform: none;
  font-size: 12.5px; color: #6b7280;}
.al-legend {display: flex; gap: 20px; font-size: 12.5px; color: #4b5157;}
.al-legend .sw {display: inline-block; width: 18px; border-top: 2.5px solid;
  margin-right: 6px; vertical-align: 3px;}
.al-legend .sw.dash {border-top-style: dashed; border-top-width: 2px;}
.al-chips {display: flex; flex-wrap: wrap; justify-content: center; gap: 8px 22px;
  font-size: 13px; font-weight: 600; color: #33373d; margin-top: 2px;}
.dot {display: inline-block; width: 9px; height: 9px; border-radius: 50%;
  margin-right: 6px;}
table.al-hold {width: 100%; border-collapse: collapse; font-size: 14px;
  animation: al-in .18s ease-out;}
table.al-hold th {text-align: left; font-size: 11px; font-weight: 600;
  letter-spacing: .07em; text-transform: uppercase; color: #6b7280;
  padding: 6px 12px 6px 0; border-bottom: 2px solid #14161a;}
table.al-hold th.num {text-align: right;}
table.al-hold td {padding: 6px 12px 6px 0; border-bottom: 1px solid #ebebe4;
  color: #1b1e23; vertical-align: middle; line-height: 1.25;}
table.al-hold td.num {text-align: right; font-weight: 600; white-space: nowrap;}
table.al-hold td.etf {font-family: ui-monospace, "SF Mono", Consolas, Menlo,
  monospace; font-size: 12.5px; color: #4b5157;}
table.al-hold td.bar {width: 32%;}
table.al-hold td.bar div {height: 5px;}
table.al-hold tbody tr:hover {background: #f4f4ee;}
table.al-hold td.dim {color: #6b7280; font-weight: 400;}
table.al-hold td.pos {color: #2f855a;}
table.al-hold td.neg {color: #c53030;}
table.al-hold td.zero {color: #9aa0a6;}
table.al-trades {font-size: 13px;}
table.al-trades th, table.al-trades td {padding-right: 18px;}
table.al-hold tr.bench td {background: #eef1f5;}
table.al-hold td .cbar {height: 4px; background: #2b6cb0; margin-top: 3px;}
table.al-hold td.yes {color: #2f855a; font-weight: 700;}
table.al-hold td.no {color: #9aa0a6;}
.al-scroll {max-height: 640px; overflow-y: auto;}
.al-scroll table.al-hold {border-collapse: separate; border-spacing: 0;}
.al-scroll table.al-hold thead th {position: sticky; top: 0; background: #fcfcfa;
  z-index: 2; border-bottom: none; box-shadow: 0 2px 0 #14161a;}
@keyframes al-in {from {opacity: 0;} to {opacity: 1;}}
...
UNIVERSE_HELP_ALL = (
    "**ETF universe** is the 14 sleeves on their actual US-listed ETFs. Its panel only "
    "becomes complete on 09 May 2019, when the managed-futures sleeve (DBMF) launched, and "
    "the engine needs 756 trading days of history before its first trade. That is why the "
...
# Only the books that carry the AI sleeve. The no-AI variants were diagnostics;
# showing them all made the picker unreadable.
SHOW_UNIVERSES = ["full_14", "us_long", "us_long_stocks"]
UNIVERSE_NAME = {
    "full_14": "ETF universe (2022-2026)",
...
def html_table(df, fmt=None, red_neg=(), bar_col=None, bench_mask=None, scroll=False,
               mono_cols=()):
    ...
    tbl = (f'<table class="al-hold"><thead><tr>{head}</tr></thead>'
           f'<tbody>{"".join(body)}</tbody></table>')
    return f'<div class="al-scroll">{tbl}</div>' if scroll else tbl


# ---------------- sidebar ----------------
with st.sidebar:
    st.markdown('<div class="al-brand">ALLOCATION LAB</div><div class="al-brandsub">'
...
if view == "Single run":
    ...
    st.markdown(f'<div class="al-kicker">{kick}</div>'
                f'<div class="al-title">{_esc(ttl)}</div>'
                f'<p class="al-desc">{_esc(DESC.get(method, ""))}</p>',
    ...
    hero = (f'<div><div class="lbl" title="Value at the end of the window">Final value</div>'
            f'<div class="big">{data.fmt_money(end_val)}</div>'
            f'<div class="sub"><span class="{pos}">{"+" if profit >= 0 else "-"}'
            f'{data.fmt_money(abs(profit))} ({(end_val / START - 1) * 100:+.1f}%)</span>'
            f' on {data.fmt_money(START)} invested</div></div>')
    if bench_eq is not None and method != BENCH:
        ...
        hero += (f'<div><div class="lbl" title="The same stake in the passive benchmark, '
                 ...
                 f'<div class="big dim">{data.fmt_money(b_end)}</div>'
                 f'<div class="sub"><span class="{gpos}">{"+" if gap >= 0 else "-"}'
                 f'{data.fmt_money(abs(gap))}</span> for this run{pts}</div></div>')
    ...
    st.markdown(f'<div class="al-hero">{hero}</div>'
                f'<div class="al-strip">{cells}</div>'
                + (f'<div class="al-note">{trade_note}</div>' if trade_note else ""),
                unsafe_allow_html=True)

    def _value_frame(eq):
        v = eq * START
        return pd.DataFrame({"date": v.index, "book_value": v.values,
    ...
    st.markdown(f'<div class="al-sec">Growth of {data.fmt_money(START)}'
                '<span class="hint">hover any date for the exact values</span></div>'
                f'<div class="al-legend">{leg}</div>', unsafe_allow_html=True)
    ...
    if rebal is not None:
        ...
        with c[0]:
            st.markdown('<div class="al-sec">Allocation<span class="hint">latest '
            ...
            plt.close(_fig)
        with c[1]:
            st.markdown('<div class="al-sec">Profit and loss by sleeve'
            ...
            if is_full:
                ...
                st.markdown('<div class="al-note">Only available over full history. '
                            'Switch Period to see it.</div>', unsafe_allow_html=True)

        hold = data.holdings(rebal, universe)
        st.markdown(f'<div class="al-sec">Holdings<span class="hint">on '
        ...
        st.markdown('<table class="al-hold"><thead><tr><th>Sleeve</th><th>Bucket</th>'
        ...
        with st.expander("Trades: every date, every sleeve"):
            ...
            for _, tr in fr.head(TRADE_ROWS).iterrows():
                ...
                rows.append(
                    ...
                    f'<td class="num">{tr[f"Trade {data.CCY}"]:,.0f}</td>'
                    f'<td class="num dim">{tr[f"Cost {data.CCY}"]:,.0f}</td></tr>')
            st.markdown(
                '<div class="al-scroll"><table class="al-hold al-trades"><thead><tr>'
...
# ---------------- compare ----------------
    ...
    st.markdown(
        f'<div class="al-kicker">{_esc(UNIVERSE_NAME.get(universe, universe))}'
        f' | {p_start:%d %b %Y} to {p_end:%d %b %Y}</div>'
        '<div class="al-title">Compare equity curves</div>'
        f'<p class="al-desc">Every run starts at {data.fmt_money(1_000_000)} on '
    ...
    if not picked:
        ...
        if wide.empty:
            ...
            leg = "".join(
                ...
                for r, c in legend)
            st.markdown('<div class="al-sec">Growth of '
                        f'{data.fmt_money(1_000_000)}<span class="hint">hover any date '
            ...
            st.markdown('<div class="al-sec">These runs, scored<span class="hint">'
                        'sorted by Sharpe, best first</span></div>',
                        unsafe_allow_html=True)
            st.markdown(html_table(show, red_neg=("Return %", "vs 60/40 %"),
...
# ---------------- scoreboard ----------------
    ...
    st.markdown(
        f'<div class="al-kicker">{_esc(UNIVERSE_NAME.get(universe, universe))}'
        f' | {p_start:%d %b %Y} to {p_end:%d %b %Y}</div>'
        '<div class="al-title">Full scoreboard</div>'
        '<p class="al-desc">Every configuration in the store, scored over the '
    ...
    st.markdown(f'<div class="al-strip" style="border-top:3px solid #14161a;'
                f'margin-top:18px">{cells}</div>'
                f'<div class="al-note">Best Sharpe: {_esc(str(best["run"]))}, '
    ...
    if b.empty:
        ...
        st.markdown(html_table(disp, red_neg=("Return %", "vs 60/40 %"),
                               bar_col="Sharpe",
...
# ---------------- which optimizer won ----------------
    ...
    st.markdown(
        '<div class="al-kicker">All windows at once | the sidebar Period '
        'does not apply here</div>'
        '<div class="al-title">Which optimizer won, and when</div>'
        '<p class="al-desc">Every method, ranked in every window.</p>',
        unsafe_allow_html=True)
    ...
    st.markdown(f'<div class="al-sec">{_esc(metric)} by window'
    ...
    st.markdown('<div class="al-sec">Winner in each window</div>',
                unsafe_allow_html=True)
    w = wins.copy()
    ...
    st.markdown(html_table(w.sort_values("Window").astype(str)),
                unsafe_allow_html=True)
    ...
    st.markdown('<div class="al-sec">Windows won per method</div>',
                unsafe_allow_html=True)
